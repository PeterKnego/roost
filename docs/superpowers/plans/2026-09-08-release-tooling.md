...
```bash
# release.yml is REGENERATED by Step 3's `dist generate` and must be committed
# with the change. Omitting it leaves the wiring in the working tree only, so a
# tag cut from this commit runs no custom job at all — and the probe would then
# report "contract does not hold" for entirely the wrong reason.
...
# A PRIVATE staging directory, and the tool is pointed at it. Both matter.
# cargo-deb resolves the `target/release/roost` asset through `cargo metadata`'s
# real target directory, not the literal string — so on a host with a shared
# target-dir (this project's CLAUDE.md documents one) staging into a local
# `target/release` writes where cargo-deb never reads, and it silently packs a
# stale cached binary. It also clobbers a developer's own `cargo build --release`
# output, which a rebuild does not repair because cargo trusts its fingerprint
...
# Private staging, for the same reason package-deb.sh uses one: cargo
# subcommands resolve asset paths through cargo metadata's real target dir.
STAGE=$(mktemp -d); trap 'rm -rf "$STAGE"' EXIT
mkdir -p "$STAGE/release"
install -m 755 "$BIN" "$STAGE/release/roost"
...
### Task 4b: Prove the packages actually install

Added 2026-09-08 at Peter's suggestion, after Task 3 shipped a `.deb` containing
the wrong binary behind four green assertions. Every check in Tasks 3 and 4 reads
*metadata*: `Depends: dtach` is present, the unit is at the right path, the binary
hashes match. None of it proves the package **works**. A misspelled dependency, an
architecture mismatch, or a version constraint nothing satisfies passes every one
of those checks and fails on a user's machine — the dev/prod substitution class
CLAUDE.md keeps a table of, applied to packaging.

**Files:**
- Create: `scripts/test/install.sh`
- Modify: `.github/workflows/ci.yml` (add a step to the existing `scripts` job)
- Modify: `Makefile` (add `test-install` to help and targets)

**Interfaces:**
- Consumes: `scripts/lib.sh`, `scripts/package-deb.sh`, `scripts/package-rpm.sh`.
- Produces: `scripts/test/install.sh` — builds the x86_64 musl binary, packages it,
  and installs both packages in containers.

- [ ] **Step 1: Write the test**

`scripts/test/install.sh`. Note it uses a REAL musl binary, not the shell-script
fixture the metadata tests use: the point is to run the shipped artifact on a
distro older than the build host.
...
docker run --rm -v "$TMP:/pkgs:ro" debian:12 sh -euc '
  ...
  roost --version
' | tail -1 | grep -qx "roost $VERSION" || die "debian:12 install failed"
...
docker run --rm -v "$TMP:/pkgs:ro" fedora:41 sh -euc '
  ...
  test -f /usr/lib/systemd/user/roost.service || { echo "unit missing"; exit 1; }
  roost --version
' | tail -1 | grep -qx "roost $VERSION" || die "fedora install failed"
...
Run: `sg docker -c scripts/test/install.sh` (or plain `scripts/test/install.sh`
if your login already has the docker group).
Expected: `[build]`, `[debian:12]` and `[fedora]` blocks, all green.

- [ ] **Step 3: Prove it can fail, twice**

The assertions must be shown red, not assumed:

1. Temporarily change `depends = "dtach"` to `depends = "dtach-nonexistent"` in
   `Cargo.toml`'s `[package.metadata.deb]`, rerun, confirm the debian block fails
   because apt cannot resolve the dependency. Restore. This is the exact defect
   the metadata tests cannot see — they would still report `Depends:` present.
2. Temporarily point `[package.metadata.deb]`'s unit asset at a copy without
   `KillMode=process`, rerun, confirm the debian block fails on the grep. Restore.

Back files up with `cp` before editing and restore from the copy — not with
`git checkout`, which has discarded uncommitted work in this repo before.

Paste both failing outputs and the restored passing run into your report.

- [ ] **Step 4: Wire it into CI and the Makefile**

Append to the `scripts` job in `.github/workflows/ci.yml`, after the existing
package-tests step:

```yaml
      # Runners have docker and the musl target is a rustup add away.
...
In the `Makefile`, add `test-install` to `.PHONY`, a help line
`@echo 'make test-install           install the packages in containers'`, and:
...
```bash
git add scripts/test/install.sh .github/workflows/ci.yml Makefile
git commit -m "release: prove the packages install, not just that they describe themselves"
```
