...
- The deploy host is reached as `ssh claude@77.42.80.36` (the tailnet name works
...
```bash
ssh claude@77.42.80.36 'systemctl --user stop deadlight; systemctl --user disable deadlight; \
...
```bash
ssh claude@77.42.80.36 'cat > ~/.config/systemd/user/resh.service <<UNIT
[Unit]
...
```bash
ssh claude@77.42.80.36 'mv /home/claude/projects/deadlight /home/claude/projects/resh && \
...
```bash
ssh claude@77.42.80.36 'systemctl --user start resh; sleep 4; \
...
```bash
ssh claude@77.42.80.36 'cd /home/claude/projects/resh && \
...
`git remote set-url` back, `tailscale set --hostname=ubuntu-16gb-hel1-2`, and
