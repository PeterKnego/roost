...
actually been done on `ubuntu-16gb-hel1-2`, the host still runs the old
...
- `tailscale set --hostname=resh` and moving `tailscale serve` to `:443` —
  ...
  `ubuntu-16gb-hel1-2.tail66d083.ts.net:8444` to
  `resh.tail66d083.ts.net` (no port). `allowed_origins` must be updated to
