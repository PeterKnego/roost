...
/// `resh peers` — who else is already working in this project.
///
/// Intended caller is a Claude Code `SessionStart` hook, same as `notify`,
/// but the delivery channel is the exact opposite one and for the same
/// reason. `notify` writes to `/dev/tty` *because* Claude Code captures hook
/// stdout, which would swallow its escape sequence. Here that capture IS the
/// mechanism: a `SessionStart` hook's stdout is parsed as JSON, and its
/// `additionalContext` is what puts the warning in front of the session that
/// just started. Writing to the tty instead would tell the terminal and not
/// the session. Same fact about the harness, opposite conclusion — so the
/// two must not be "made consistent".
///
/// Always exits 0. A session must never fail to start because resh could not
/// work out who else was here.
pub fn run_peers(_args: &[String]) -> i32 {
    let dir = match std::env::var_os("RESH_CLAUDE_SESSIONS") {
        Some(d) => std::path::PathBuf::from(d),
        None => match std::env::var_os("HOME") {
            Some(h) => std::path::PathBuf::from(h).join(".claude/sessions"),
            None => return 0,
        },
    };

    // A hook is always handed its payload on stdin, but a human running this
    // in a terminal is not — and `read_to_string` on a tty blocks forever.
    // A command that runs before every session starts must never be able to
    // hang, so the terminal case skips the read entirely.
    let payload: serde_json::Value = if std::io::stdin().is_terminal() {
    ...
    let self_sid = payload.get("session_id").and_then(|v| v.as_str());

    let here = match payload.get("cwd").and_then(|v| v.as_str()) {
        Some(c) => crate::peers::normalise(c),
        None => match std::env::current_dir() {
            Ok(d) => d.canonicalize().unwrap_or(d),
            Err(_) => return 0,
        },
    };
    ...
    if roots.is_empty() {
        // A misconfiguration, not a quiet "you are alone" — and distinct from
        // the ordinary case of running outside any project. stderr, so it can
        // never be mistaken for the hook's JSON payload.
        eprintln!("resh peers: RESH_ROOTS is unset, so no directory resolves to a project");
        return 0;
    }
    // Outside every root there is no project to be a peer in. Silent: this is
    // the normal case for a hook that fires in every directory on the host.
