...
//!
//! The allowlist comes from DEADLIGHT_ORIGINS or the *global* config only.
...
mod tests {
    ...
    fn allowed() -> Vec<String> {
        vec!["https://box.tail66d083.ts.net:8444".to_string()]
    }
    ...
    fn configured_origin_passes_exactly() {
        assert!(origin_allowed(Some("https://box.tail66d083.ts.net:8444"), &allowed()));
        // scheme and port are part of the origin; near-misses are not matches
        assert!(!origin_allowed(Some("http://box.tail66d083.ts.net:8444"), &allowed()));
        assert!(!origin_allowed(Some("https://box.tail66d083.ts.net:9999"), &allowed()));
    }
    ...
    fn hostile_and_missing_origins_are_rejected() {
        ...
        assert!(!origin_allowed(Some("https://evil.box.tail66d083.ts.net:8444"), &allowed()));
    }
    ...
    fn host_prefers_forwarded_then_falls_back() {
        ...
        assert!(host_allowed(
            Some("127.0.0.1:8444"),
            Some("box.tail66d083.ts.net:8444"),
            &allowed()
        ));
